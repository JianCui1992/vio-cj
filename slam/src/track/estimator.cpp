...
void Estimator::processImage(const std::map<int, std::vector<std::pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const double timestamp)
{
    ...
    all_image_frame.insert(std::make_pair(timestamp, imageframe));
    ...
    if(ESTIMATE_EXTRINSIC == 2)
    {
        ...
        if (frame_count != 0)
        {
            ...
            if (initial_ex_rotation.CalibrationExRotation(corres, pre_integrations[frame_count]->delta_q, calib_ric))
            {
                ...
                // //printf("initial extrinsic rotation: " << endl << calib_ric);
                ric[0] = calib_ric;
                RIC[0] = calib_ric;
    ...
    if (solver_flag == INITIAL)
    {
        ...
        {
            bool result = false;
            if( ESTIMATE_EXTRINSIC != 2 && (timestamp - initial_timestamp) > 0.1)
            {
            ...
            std::count(std::begin(last_marginalization_parameter_blocks), std::end(last_marginalization_parameter_blocks), para_Pose[WINDOW_SIZE - 1]))
        {
            ...
            if (last_marginalization_info)
            {
                ...
                for (int i = 0; i < static_cast<int>(last_marginalization_parameter_blocks.size()); i++)
                {
                    ...
                    {
                        //printf("Error in estimator.cpp line 941 \n");
                        getchar();
                    }
